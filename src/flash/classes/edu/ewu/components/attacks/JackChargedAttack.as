package edu.ewu.components.attacks 
{
	...
	public dynamic class JackChargedAttack extends Attack
	{
		...
		public function JackChargedAttack($oCreator:Player, $nX:uint, $nY:uint, $nAngle:uint, $nForce:uint=0, $nDamage:uint=0, $bNetwork:Boolean = false) 
		{
			...
			TweenMax.to(this, this._nTime/2, { x:$nX + 200 * Math.cos(this.angle / 180 * Math.PI), y:$nY + 200 * Math.sin(this.angle / 180 * Math.PI), rotation:360, onComplete:tweenBack } );
		}
		
		private function tweenBack():void 
		{
			this.angle = this.angle > 180? this.angle - 180 : this.angle + 180;
			TweenMax.to(this, this._nTime/2, { x:this.x + 200 * Math.cos(this.angle / 180 * Math.PI), y:this.y + 200 * Math.sin(this.angle / 180 * Math.PI), rotation:360} );
		}
